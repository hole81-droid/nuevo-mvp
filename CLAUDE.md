...
외부 SNS에서 들어온 유저는 첫 앱을 체험한 뒤 검색을 배우기 전에 다음 앱을 바로 이어서 만난다.
...
2. 외부 유입 후 Play-first Vertical Stack: 첫 앱 아래로 다음 추천 앱이 바로 이어짐
3. URL 업로드 + iframe 호환성 진단 + 모바일 미리보기
4. 비로그인 앱 체험
5. 태그 검색 / 카테고리 탐색
6. 리믹스 생성 + 원본 상세 노출 + `N회 리믹스됨` 소셜 증명
7. 반응 / 댓글 / 저장 / 공유
8. 창작자 Fame 대시보드: 체험 수, 평균 체험 시간, 반응, 저장, 공유, 리믹스, 유입 채널
9. WES breakdown / raw event log export
...
- `utm_source=ig|tt_bio|yt_shorts` 같은 실제 소셜 링크 별칭도 표준 유입 채널로 정규화한다.

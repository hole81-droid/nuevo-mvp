...
- [ ] **N1. 앱 상세 딥링크 라우트 정리**
  - [ ] `@handle/app-slug` 형태의 공유 URL 설계 또는 현재 라우트와 매핑
  - [x] `?autoplay=true` 지원
  - [x] 딥링크 진입 시 iframe 자동 전개
  - [ ] 모바일 LCP 3초 이하 목표로 QA

- [ ] **N2. 공유 미리보기 / 공유 버튼**
  - [ ] 앱 상세별 OG title / description / image 설정
  - [ ] 앱 미리보기 이미지 fallback 정의
  - [x] 딥링크 복사 버튼
...
- [ ] **N11. `/studio`를 Fame 대시보드 중심으로 재정렬**
  - [ ] 체험 수
  - [ ] 평균 체험 시간
  - [ ] 반응 수
  - [ ] 댓글 수
...
- [ ] **N12. 유입 채널 분석**
  - [ ] 딥링크에 `utm_source` 또는 referrer 기반 source 기록
  - [ ] Instagram / TikTok / YouTube / Reddit / Direct 분류
  - [ ] 앱별 유입 채널 breakdown 표시

### P1 — WES 신뢰 장치

- [ ] **N13. WES breakdown / raw event export**
  - [ ] 세션/시간/반응/댓글/리믹스 항목별 WES 분해 표시
  - [ ] 월별 raw event log CSV export
  - [ ] "예상 수익"과 "확정 수익" 문구 분리
